...
const MePopUp = {
  title: "About Me",
  text: "hi"  , imArray: [{src: "src/assets/images/selfie.png", alt: "HEJ"}]
}
const ContactsPopUp = {
  title: "Contact Me",
  text: "GRRRR!",
  imArray: [{src: "src/assets/images/hero.jpg", alt: "HEJ"}]
}
...
const Comp_Canvas = () => {
  ...
  const MirrorPosition = new THREE.Vector3(-8, -8, -7);
  const MirrorPosition2 = new THREE.Vector3(0, 0, 0);
  const ComputerPosition = new THREE.Vector3(14, 3, -10);
  const ComputerAngle = new THREE.Vector3(0, 0, 0);
  const PaperPosition = new THREE.Vector3(0, 20, 2);
  const PaperAngle = new THREE.Vector3(-4, 12, -10);

  const SideBar = ({ text1, text2, text3, handleButtonClick}) => {
    const sidebarClass = `fixed top-0 ${isSmallScreen ? 'left-3' : 'right-0'} h-screen w-16 m-0 flex ${isSmallScreen ? 'flex-row' : 'flex-col'} bg-transparent text-black shadow-lg z-0`;
    const sidebarItemClass = `m-2 ${isSmallScreen ? 'mr-3' : 'mb-3'}`;

    return (
      <div className={sidebarClass}>
        <SideBarIcon className={sidebarItemClass} icon={<FaSquarePhone size={50} />} text={text1} downIcon={isSmallScreen} onClick={() => handleButtonClick(MirrorPosition, MirrorPosition2, ContactsPopUp)}  />  
  ...
  const [popupContent, setPopupContent] = useState({
    ...
    imArray: [{src: "src/assets/images/hero.jpg", alt: "HEJ"}, {src: "src/assets/images/hero.jpg", alt: "HEJ"}]
  });
  
  return (
    <div className = {`flex ${isSmallScreen ? 'flex-col-reverse h-screen' : 'flex-row h-screen'}`}> 
      <div className={`relative ${isSmallScreen ? 'h-2/3 w-full' : 'w-2/3 h-full'} background-color-black`}>
        {showPopup && (
          <BackButton onClick={() => {
            handleButtonClick(MirrorPosition, MirrorPosition2);
          }} />
        )}
        ...
        >
          ...
          <Suspense fallback={<Loader />}>
            <Model />
            <Button position={[-1.8, 1.5, 20.1]} shape={[4,6,0.1]} onClick={() => handleButtonClick(MirrorPosition, MirrorPosition2,  ContactsPopUp)} controlsEnabled={controlsEnabled}  />
            ...
            <Arrow positionArr={[-16,-7,-3]} color={"white"} rotation={[0, 0, (3/2)*Math.PI]}/>
            <Arrow positionArr={[-0.5,-18,7]} color={"Deeppink"} rotation={[0, (1/2)*Math.PI, 0]} nr={2}/>
            <Arrow positionArr={[-2,-3.5, 19.4]} color={"5BB9C9"} rotation={[0, Math.PI/2, Math.PI/2]} nr={3}/>
            <OrbitControls
              enableZoom={false}
      ...
      <div >
        ...
        <SideBar text1="Contact" text2="Portfolio" text3="About Me" handleButtonClick={handleButtonClick}/>
      ...
      <div className={`${isSmallScreen ? 'h-1/3 w-full top-0' : 'w-1/3 h-full top-0'} bg-black text-white p-9 overflow-y-hidden`}>
        ...
        <p className="text-sm text-gray-500 font-lato font-bold mb-8 z-20" >
